...
class TimeManager:
    ...
    def set_callback_on_day_change(self, callback:Callable):
        if callback is None:
            logger.warning("Can't set none as callback")
            return
        if not callable(callback):
            logger.warning(f"Can't register non callable callback: {callback}")
            return
